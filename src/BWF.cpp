...
struct BrickWall {
  ...
  void process(gam::STFT& stft) {
    if(maxFreq>=12000) maxFreq=24000;
...
struct BWF : Module {
  ...
  enum FFTLEN { FL1024,FL2048,FL4096};
  int fftLen=1;
  BrickWall bw;
  ...
  void onAdd(const AddEvent &e) override {
    ...
    for(int c=0;c<channels;c++) {
      ...
      float out=0.f;
      switch(fftLen) {
        case FL1024:out=fd1024[c].process(in,bw);break;
        case FL2048:out=fd2048[c].process(in,bw);break;
        case FL4096:out=fd4096[c].process(in,bw);break;
        default: break;
      }
