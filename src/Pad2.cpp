  ...
  void randomizePartials(int method) {
    switch(method) {
      case EVEN_MIN:
        for(int k=0;k<48;k++) {
          if(k%2==1)
            setImmediateValue(getParamQuantity(PARTIAL_PARAM+k),rnd.nextMin(k/4));
          else
            setImmediateValue(getParamQuantity(PARTIAL_PARAM+k),0);
        }
        break;
      case ODD_MIN:
        for(int k=0;k<48;k++) {
      ...
      case HARMONIC_WB:
        for(int k=0;k<48;k++) {
          setImmediateValue(getParamQuantity(PARTIAL_PARAM+k),rnd.nextWeibull(k/4.f));
        }
        break;
      case EVEN_WB:
        for(int k=0;k<48;k++) {
    ...
    for(int k=0;k<3;k++) {
      if(inputs[PARTIALS_INPUT+k].isConnected()) {
        for(int chn=0;chn<16;chn++) {
          ...
          setImmediateValue(getParamQuantity(PARTIAL_PARAM+k*16+chn),newParam);
        }
      }
    }
