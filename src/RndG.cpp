...
struct RndG : Module {
  ...
  dsp::PulseGenerator clkPulseGenerators[16];
  bool lastCoins[16] = {};
  ...
  void next(int channels) {
    ...
    for(int k=0;k<channels;k++) {
      bool trigger=clkPulseGenerators[k].process(1.0/args.sampleRate);
      outputs[CLK_OUTPUT].setVoltage((trigger?10.0f:0.0f),k);
      trigger=trgPulseGenerators[k].process(1.0/args.sampleRate);
