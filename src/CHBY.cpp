...
template<typename T>
struct ChebyFilter {
  T xn[CL_LIM],yn[CL_LIM], xnm1[CL_LIM], xnm2[CL_LIM], ynm1[CL_LIM], ynm2[CL_LIM];
  float alpha[CL_LIM], beta[CL_LIM];
  float b0[CL_LIM], b1[CL_LIM], b2[CL_LIM];
  T a0[CL_LIM], a1[CL_LIM], a2[CL_LIM];
  int nsec=4;
  void init(bool highPass=false) {
    ...
    for(int m=0;m<=nsec-1;m++) {
      ...
      if (m==0) { /* H0 and pole magnitudes */
        b0[m] = ((alpha[m])*(alpha[m]) +
                    (beta[m])*(beta[m]))*1.0f/sqrtf(1.0f+eps*eps);
        b1[m] = (highPass?-1.f:1.f)*((alpha[m])*(alpha[m]) +
                    (beta[m])*(beta[m]))*2.0f/sqrtf(1.0f+eps*eps);
        b2[m] = ((alpha[m])*(alpha[m]) +
                    (beta[m])*(beta[m]))*1.0f/sqrtf(1.0f+eps*eps);
      }
...
struct CHBY : Module {
  ...
  ChebyFilter<float_4> hpFilter[4];
    ...
    if(lpConnected) outputs[LP_OUTPUT].setChannels(channels);
    if(hpConnected) outputs[HP_OUTPUT].setChannels(channels);
	}
};
